...
main은 릴리스 기준, dev는 개발 통합이다. 일반 작업은 최신 origin/dev 기반 type/english-kebab-case 브랜치에서 수행한다. 현재 origin/dev가 없어 최초 관리자 설정이 남았다. 기존 원격 초기 이력은 ADR0002대로 보존한다. 커밋이 전혀 없는 저장소만 기능 없는 최소 bootstrap 한 개로 기준을 만들 수 있으며 이 저장소에는 적용하지 않는다.
...
5. 검증 후 명시적으로 작업 브랜치 push와 draft PR을 준비할 수 있다. 현재 요청에서는 원격 변경을 하지 않는다. dev로 squash merge, squash 제목/본문도 한국어로 편집. 승인 없는 자체 병합 금지.
...
현재 보호 미적용. 권한은 조회로 확인했지만 설정 적용·dev 생성·원격 push는 승인 범위에 포함하지 않았다. 보호 파일 존재와 실제 서버 적용은 별개다.
