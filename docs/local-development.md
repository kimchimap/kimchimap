...
지정·매칭 관리는 로그인된 ADMIN의 관리 메뉴에서 연다. 지정 source 허가가 없으면 등록 불가가 정상이다. 실제 협회 허가는 미확보이며 테스트의 가상 허가 소스를 운영 데이터에 주입하지 않는다. 기존 매칭 관찰본이 없으면 허용된 수집 재실행으로 자료를 확보한 뒤 비교한다.
