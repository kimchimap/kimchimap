...
무필터 지도·반경, 직접 상세, 즐겨찾기, 공개 이미지에서 국내산 근거가 없는 업소를 제외한다. 마지막 근거 철회·만료·비공개·상충을 검증하고, 다른 유효 국내산 품목이 있을 때 수입산·혼합·분쟁 정보도 원래 범위대로 표시되는지 확인한다. 전화번호·제보 테스트는 명시적인 별도 국내산 쌀밥 합성 자료로 기존 공개 업소를 구성하며, 검사 대상 김치 기록 수는 쌀밥과 구분한다.

기존 수집 엔진의 멱등·잠금·재시도·요청 제한 테스트는 합성 클라이언트와 수집 자격 테스트 대역으로 보존한다. 이는 실서비스의 일반음식점 수집 허용을 의미하지 않는다. 별도 DomesticPublicationIntegrationTest는 실제 자격 정책과 실제 DB로 신규·예약·기존 대기 작업이 차단되고 외부 클라이언트가 호출되지 않는지 검증한다. 검사 삭제·건너뛰기 없이 변경된 제품 수용 기준을 반영한다.
