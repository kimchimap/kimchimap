...
- 브랜치 feat/origin-corrections, 기준 dev df40cf6. 관리자 수집 PR #10 병합·임시 브랜치 삭제·원래 dev 동기화 완료.
- 초기 실제 HTTP 검색 검증에서 includeMixed 생략이 400으로 거부되는 문제를 발견했다. nullable Boolean을 false로 정규화하고 모든 검색 응답 200을 검사한 뒤 결과를 검증하도록 보강했다. 좌표 필수 검증과 원산지 의미는 완화하지 않았다.
- 원산지 정정 전 분쟁 제외, 잘못된 기록 철회 후 국내산 검색 매칭, 새 상충 주장 후 재제외를 실제 PostGIS·HTTP로 확인했다. 이전 기록·감사 불변, 타 그룹/일반 사용자 거부와 오래된 버전 충돌도 검사했다.
- 최종 ./scripts/harness verify exit 0. /tmp/kimchimap-origin-corrections-verify.log에 기록했다. 단위·계층 37, 통합 61, 외부 계약 5, 성능 1, 프론트 32, 모바일·PC 브라우저 12, 하네스 28건 통과. 실패·오류·skip 0. 포맷·린트·타입·계약 일치·빌드 통과. 변경 없는 Gradle 작업은 캐시를 사용했다.

## 지정·업소 매칭 관리 (2026-09-08)

- 브랜치 feat/admin-data-review, 기준 dev 3de6d41. 원산지 정정 PR #11 병합·임시 브랜치 삭제·원래 dev 동기화 완료.
- 서로 다른 외부 식별자를 같은 업소에 동시에 연결하는 경쟁 조건을 검토 중 발견했다. 대상 업소 행 잠금 후 기존 출처 식별자를 재조회하도록 보강했다. 실제 동시 HTTP 요청에서 한 건 200·다른 건 409와 외부 ID 한 건만 저장됨을 확인했다.
- 최종 ./scripts/harness verify exit 0. /tmp/kimchimap-data-review-verify.log에 기록했다. 단위·계층 37, 실제 PostGIS·Redis 통합 65, 외부 계약 5, 성능 1, 프론트 38, 모바일·PC 브라우저 12, 하네스 28건 통과. 실패·오류·skip 0. 포맷·린트·타입·API 계약 일치·양쪽 빌드 통과. 변경 없는 Gradle 결과는 캐시를 사용했다.
- 지정 허가 기본 거부·이력 불변·원산지 비생성·공개 허가 철회와 매칭 관찰본·수집 시각 보존을 확인했다. 실제 협회 자료와 사용자 자료는 사용하지 않았다. 전체 검사 중 라디오 입력의 스타일을 정리했으며 최종 커밋을 PR 준비 검사에서 다시 검증한다.

## 국내산 업소 공개 범위 정정 · 2026-09-08

- 브랜치 fix/domestic-publication-scope, 기준 dev 8909dc3599a518a6b7e83e146d84544a41b76d29.
- 전체 verify 종료 코드 0. 백엔드 단위·계층 37, 실제 PostGIS·Redis 통합 67, 외부 합성 계약 5, 공간 실행계획 1, 프론트 39, 모바일·PC E2E 12, 하네스 28건 통과. 실패·오류·건너뜀 0. 실제 OpenAPI 타입 일치와 양쪽 빌드 통과.
- 무근거 업소 공개를 기대하던 기존 테스트를 제품 기준에 맞게 바꿨다. 독립 기능 검증은 별도 국내산 합성 품목을 명시적으로 추가했고, 수집 엔진 계약과 실제 자격 정책 차단 검증을 분리했다. 테스트를 삭제하거나 건너뛰지 않았다. 첫 통합 실행 34건 실패, 자료·기대값 수정 후 14건, 수정 후 2건 실패를 해결했다.
- 공간 합성 자료 20,000업소·20,000원산지, 로컬 Apple Silicon의 AMD64 PostGIS 에뮬레이션. 반경 57.338ms, 영역 2.223ms의 해당 단일 실행 측정이며 각각 geography/geometry GiST 사용 확인. 처리량·운영 지연 보장이 아니다. 상세 계획은 로컬 backend/build/reports/performance/spatial.json.
- 로컬 DB에 열한 번째 마이그레이션 적용. 과거 수집 작업 4947bcc0-2d59-41a1-90fe-7eee6dac77f6 상태 CANCELLED, 기본정보 저장 199개 중 국내산 공개 자격 0개. 과거 자료를 삭제하지 않고 공개 경로에서 제외했다. ingest-run은 원산지 선별 불가능 정책으로 의도한 종료 코드 1. 새로운 외부 데이터 호출·수집은 하지 않았다.
- 로그: /tmp/kimchimap-domestic-verify.log, /tmp/kimchimap-domestic-live-status.log, /tmp/kimchimap-domestic-live-rejected.log. 로그와 실제 자료는 Git에 커밋하지 않는다.
- 실제 원산지 소스 확보·협회 허가·미등록 제보 흐름·운영 준비 완료를 뜻하지 않는다. 배포·CI/CD 없음.
