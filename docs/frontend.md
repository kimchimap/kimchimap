...
관리자 수집 화면 /admin/ingestion은 소스 허가·키 설정 여부·정기 실행 상태와 최근 50개 작업을 표시한다. 15초 주기로 상태를 갱신하고 페이지 처리·격리 목록은 별도 페이지로 가져온다. 일부 처리와 전체 조회 범위 완료를 구분하고 재실행은 기본 2페이지, 최대 100페이지와 사유를 요구한다.

관리자 /admin/origins에서 판정 상태별 목록과 동일 품목·용도·식재료 근거를 검토한다. 증빙은 원래 제보로 연결한다. 제외할 기록과 사유를 선택하며 버전 충돌 시 입력을 보존하고 최신 근거를 다시 확인한다. 새 값은 제보·검수 경로로 등록한다. 관리자 진입 경계는 MemberGate로 공통화했다.

관리자 /admin/designations는 지정 이용 허가가 없으면 등록 양식을 제공하지 않는다. 허가된 출처가 있을 때 업소 상세에서 진입해 적용 품목·기준·지정/만료/취소일·사유를 기록한다. /admin/matches는 수집 관찰본과 후보 지점을 비교하며 기존 연결 또는 별도 업소 생성만 명시적으로 결정한다. 입력 충돌은 내용을 보존한다. 회원 메뉴의 관리 항목은 키보드로 여는 접이식 메뉴다.
