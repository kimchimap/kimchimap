...
제보 구현 화면: 업소 연락처에서 /reports/new?restaurantId=…로 진입한다. /reports와 /reports/:id에서 상태·검토 사유·비공개 사진을 확인하고 대기·보완 상태만 수정·철회한다. 관리자 /admin/reports와 상세에서 품목·용도·원문·관찰일을 검토하고 사진별 공개 검토를 선택한다. 응답 실패 시 신규 제출 키를 유지하며 충돌은 최신 상태 확인을 안내한다. 사진 Blob URL은 화면 정리 시 폐기한다. 관리자 화면 접근 제어와 서버 인가는 별도로 적용한다.
