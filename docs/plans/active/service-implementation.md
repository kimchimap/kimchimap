...
현재 브랜치: fix/domestic-publication-scope. 기준 origin/dev: 8909dc3599a518a6b7e83e146d84544a41b76d29. 하네스 PR #1 병합과 임시 브랜치 삭제를 완료했다.
...
P07a PR #8 병합과 임시 브랜치 삭제를 완료했다. P07b 제보·검수 API와 화면을 구현하고 전체 검증을 통과했다. PR #9 병합·임시 브랜치 삭제를 완료했다. 관리자 수집 관리 작업을 이어서 진행한다. 세부 기록은 reports-review.md를 따른다.

관리자 수집 PR #10 병합·임시 브랜치 삭제 완료. PR 준비 검증은 추가 정리 테스트를 포함해 통합 59건을 통과했다. 원산지 정정 API·화면과 실제 검색 반영을 구현하고 전체 검증을 진행한다.

원산지 정정 PR #11 병합·임시 브랜치 삭제 완료. 현재 지정 정보와 업소 매칭의 관리자 검토를 구현하고 검증한다. 협회 허가는 여전히 없으며 이 작업으로 실제 지정 자료를 수집하지 않는다. 다음 작업은 P08 지도·목록·상세 전체 화면이다.

2026-09-08 사용자 범위 정정: 국내산 사용 항목이 확인된 업소만 수집·공개한다. PR #12 지정·매칭 병합은 완료했다. 지도 구현은 feat/map-experience의 09dc0da에 미병합 보존한다. 현재 공개 자격·원산지 없는 전체 수집 차단을 우선 수정한다. P05는 기술적 수집 엔진 구현 이력이며 적합한 원산지 데이터 확보 완료가 아니다. 다음 순서는 공개 자격 수정 검증·PR → 미등록 업소 제보·검수·공개 → 지도 통합 → 남은 계정 화면·실제 연결 E2E·최종 검증이다.
